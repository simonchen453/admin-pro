 ...
 */
@Component
public class MenuUpdateValidator extends BaseValidator<MenuEntity> {
    ...
    public void validate(MenuEntity entity, MessageBundle msgBundle) {
        ...
        if (!msgBundle.hasErrorMessage("id")) {
            if (!StringUtils.isEmpty(entity.getId())) {
                MenuEntity menuEntity = menuService.findById(entity.getId());
                if (menuEntity == null) {
                    msgBundle.addErrorMessage("id", "菜单权限不存在");
                }
            }
        }
