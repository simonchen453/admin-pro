...
export const getJobDetailApi = async (id: string): Promise<JobDetailResponse> => {
  const response = await request.get<ApiResponse<JobDetailResponse>>(`/api/v1/tools/jobs/${id}`);
...
export const createJobApi = async (jobData: JobEntity): Promise<ApiResponse> => {
  const response = await request.post<ApiResponse>('/api/v1/tools/jobs', jobData);
  return response as unknown as ApiResponse;
};

// 更新定时任务
export const updateJobApi = async (jobData: JobEntity): Promise<ApiResponse> => {
  const response = await request.put<ApiResponse>(`/api/v1/tools/jobs/${jobData.id}`, jobData);
  return response as unknown as ApiResponse;
};

// 删除定时任务
export const deleteJobApi = async (ids: string): Promise<ApiResponse> => {
  const response = await request.delete<ApiResponse>(`/api/v1/tools/jobs?ids=${ids}`);
  return response as unknown as ApiResponse;
};

// 暂停任务
export const pauseJobApi = async (id: string): Promise<ApiResponse> => {
  const response = await request.patch<ApiResponse>('/api/v1/tools/jobs/pause', id);
  return response as unknown as ApiResponse;
};

// 立即执行任务
export const runJobApi = async (id: string): Promise<ApiResponse> => {
  const response = await request.patch<ApiResponse>('/api/v1/tools/jobs/run', id);
  return response as unknown as ApiResponse;
};

// 重启任务
export const resumeJobApi = async (id: string): Promise<ApiResponse> => {
  const response = await request.patch<ApiResponse>('/api/v1/tools/jobs/resume', id);
  return response as unknown as ApiResponse;
};

// 获取下次执行时间
export const getNextTimeApi = async (cronExpression: string): Promise<JobNextTimeResponse> => {
  const response = await request.get<ApiResponse<JobNextTimeResponse>>('/api/v1/tools/jobs/nextTime', {
...
export const deleteJobLogApi = async (ids: string): Promise<ApiResponse> => {
  const response = await request.delete<ApiResponse>(`/api/v1/tools/jobs/logs?ids=${ids}`);
  return response as unknown as ApiResponse;
};

// 清空所有定时任务日志
export const deleteAllJobLogApi = async (): Promise<ApiResponse> => {
  const response = await request.delete<ApiResponse>('/api/v1/tools/jobs/logs/all');
  return response as unknown as ApiResponse;
};
